...
class ShapeFillParser {
    ...
    static ShapeFill parse(JsonReader reader, LottieComposition composition) throws IOException {
        ...
        AnimatableIntegerValue opacity2 = opacity == null ? new AnimatableIntegerValue(Collections.singletonList(new Keyframe(100))) : opacity;
        Path.FillType fillType = fillTypeInt == 1 ? Path.FillType.WINDING : Path.FillType.EVEN_ODD;
        return new ShapeFill(name, fillEnabled, fillType, color, opacity2, hidden);
    }
}
