...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1926plusAssignLRDsOJo(long duration) {
        long longDelta = Duration.m1836toLongimpl(duration, getUnit());
        ...
        if ($i$f$isSaturated == 0) {
            ...
            if ((this.reading ^ longDelta) >= 0 && (this.reading ^ newReading) < 0) {
                m1925overflowLRDsOJo(duration);
            }
            this.reading = newReading;
            return;
        }
        long half = Duration.m1793divUwyO8pc(duration, 2);
        long $this$isSaturated$iv = Duration.m1836toLongimpl(half, getUnit());
        if (!((($this$isSaturated$iv - 1) | 1) == Long.MAX_VALUE)) {
            long readingBefore = this.reading;
            try {
                m1926plusAssignLRDsOJo(half);
                m1926plusAssignLRDsOJo(Duration.m1825minusLRDsOJo(duration, half));
                return;
    ...
    private final void m1925overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + DurationUnitKt.shortName(getUnit()) + " is advanced by " + ((Object) Duration.m1839toStringimpl(duration)) + '.');
    }
}
