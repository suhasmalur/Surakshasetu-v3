...
class MotionPaths implements Comparable<MotionPaths> {
    ...
    void initPath(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float startCenterX2 = -pathVectorY;
        float normalX = startCenterX2 * perpendicular;
    ...
    void getCenter(double p, int[] toUse, double[] data, float[] point, double[] vdata, float[] velocity) {
        ...
        if (this.mRelativeToController != null) {
            ...
            float v_y3 = (float) ((((double) ry) - (((double) radius) * Math.cos(angle))) - ((double) (v_height / 2.0f)));
            float dpos_x = (float) (((double) v_x2) + (((double) dradius) * Math.sin(angle)) + (Math.cos(angle) * ((double) dangle2)));
            dpos_y = (float) ((((double) v_y2) - (((double) dradius) * Math.cos(angle))) + (Math.sin(angle) * ((double) dangle2)));
            dangle = dpos_x;
            v_y = v_y3;
            v_x = v_x3;
        }
    ...
    void setView(float position, View view, int[] toUse, double[] data, double[] slope, double[] cycle, boolean mForceMeasure) {
        ...
        if (this.mRelativeToController != null) {
            ...
            float drx = vel[0];
            float dry = vel[1];
            float angle = path_rotate;
            float pos_x = (float) ((((double) rx) + (((double) radius) * Math.sin(v_y4))) - ((double) (v_width2 / 2.0f)));
            float pos_y = (float) ((((double) ry) - (((double) radius) * Math.cos(v_y4))) - ((double) (v_height2 / 2.0f)));
            v_height = v_height2;
            dangle = v_width2;
            float dpos_x = (float) (((double) drx) + (((double) dradius) * Math.sin(v_y4)) + (((double) radius) * Math.cos(v_y4) * ((double) dangle2)));
            float dpos_y = (float) ((((double) dry) - (((double) dradius) * Math.cos(v_y4))) + (((double) radius) * Math.sin(v_y4) * ((double) dangle2)));
            v_x = pos_x;
            if (slope.length < 2) {
                z2 = false;
                z = true;
            ...
            if (!Float.isNaN(angle)) {
                dv_y = dpos_y;
                float rot = (float) (((double) angle) + Math.toDegrees(Math.atan2(dpos_y, dpos_x)));
                view2 = view;
            ...
            if (!Float.isNaN(path_rotate2)) {
                ...
                float dy = dv_y3 + (dv_height3 / 2.0f);
