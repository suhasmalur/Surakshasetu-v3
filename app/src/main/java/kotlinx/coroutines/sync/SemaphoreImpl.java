...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cont) {
        ...
        Object objM2019constructorimpl;
        Object s$iv;
        boolean z;
            ...
            while (true) {
                if (cur$iv$iv.getId() < id$iv || cur$iv$iv.getRemoved()) {
                    ...
                    if (it$iv$iv$iv == ConcurrentLinkedListKt.CLOSED) {
                        objM2019constructorimpl = SegmentOrClosed.m2019constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    objM2019constructorimpl = SegmentOrClosed.m2019constructorimpl(cur$iv$iv);
                    curTail = s$iv2;
                    break;
                }
            }
            s$iv = objM2019constructorimpl;
            if (SegmentOrClosed.m2024isClosedimpl(s$iv)) {
                break;
            }
            Segment to$iv$iv = SegmentOrClosed.m2022getSegmentimpl(s$iv);
            while (true) {
        ...
        SemaphoreSegment segment = (SemaphoreSegment) SegmentOrClosed.m2022getSegmentimpl(s$iv);
        int i = (int) (enqIdx % ((long) SemaphoreKt.SEGMENT_SIZE));
        if (!Striped$SmallLazyStriped$$ExternalSyntheticBackportWithForwarding0.m(segment.acquirers, i, null, cont)) {
            Object expected$iv = SemaphoreKt.PERMIT;
            Object value$iv = SemaphoreKt.TAKEN;
            if (Striped$SmallLazyStriped$$ExternalSyntheticBackportWithForwarding0.m(segment.acquirers, i, expected$iv, value$iv)) {
                cont.resume(Unit.INSTANCE, this.onCancellationRelease);
                return true;
            }
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        if (value$iv == null) {
            ...
            Object expected$iv = SemaphoreKt.PERMIT;
            return !Striped$SmallLazyStriped$$ExternalSyntheticBackportWithForwarding0.m(segment.acquirers, i, expected$iv, SemaphoreKt.BROKEN);
        }
